...
const GoogleReviews = () => {
  ...
  const mainReview = {
    ...
    rating: 4.8,
    totalReviews: 878,
    avatar: "https://cdn-fikdi.nitrocdn.com/jMPRxdBtzdyhpLnQAckZGMhPAXVBimeo/assets/desktop/optimized/rev-ca42595/lh3.googleusercontent.com/places/AJDFj42XKGjpeF4xYYXXA-wEWY1c4lQrjKwjdnKUztaBeHAFTNZWXKyauoESTNP0SH-KFmG0GUN8SJ0uOxQSZV2XOlqXz9q4KQlB1HA=s1600-w300-h300"
  };
    ...
    <div className="w-full py-8 md:py-16 bg-gray-50">
      <div className="max-w-[1320px] mx-auto px-4">
        <div className="flex flex-col lg:flex-row items-start justify-between gap-8">
          {/* Main Review Card */}
          <div className="w-full lg:w-[400px] p-6 bg-white rounded-lg shadow-md">
            <div className="flex gap-4">
              ...
              <div className="flex-1 min-w-0"> {/* Added min-w-0 to prevent text overflow */}
                <h3 className="text-[15px] font-medium text-gray-800 leading-tight mb-2 break-words">
                  {mainReview.name}
                </h3>
          ...
          <div className="relative w-full lg:w-[calc(100%-432px)]"> {/* Adjusted width for desktop */}
            <div className="overflow-hidden">
              ...
              >
                {reviews.map((review, index) => (
                  <div 
                    key={index}
                    className="w-full min-w-full flex-shrink-0 p-4 sm:p-6 bg-white rounded-lg shadow-md"
                  >
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        <h4 className="text-sm sm:text-base font-medium text-gray-800">{review.name}</h4>
                        <div className="flex items-center gap-2">
                          <div className="flex gap-[2px]">
                            {[...Array(review.rating)].map((_, i) => (
                              <svg key={i} className="w-3 h-3 sm:w-4 sm:h-4 text-yellow-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                              </svg>
                            ))}
                          </div>
                    ...
                    <p className="text-sm sm:text-base text-gray-600 line-clamp-4">{review.review}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <button 
              onClick={prevSlide}
              className="absolute -left-3 top-1/2 -translate-y-1/2 bg-white rounded-full p-1.5 sm:p-2 shadow-md hover:bg-gray-100 z-10"
            ...
            <button 
              onClick={nextSlide}
              className="absolute -right-3 top-1/2 -translate-y-1/2 bg-white rounded-full p-1.5 sm:p-2 shadow-md hover:bg-gray-100 z-10"
